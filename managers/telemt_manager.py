...
"""
    ...
    def add_client(self, protocol_type, name, host='', port='', **kwargs):
        ...
        if link == "Not found":
            link = self._fallback_link(secret, host, port)
    ...
    def get_client_config(self, protocol_type, client_id, host='', port=''):
        ...
        if c:
            ...
            if secret: return self._fallback_link(secret, host, port)
        return "Not found"

    def _fallback_link(self, secret, host, port):
        config = self._get_server_config()
        match = re.search(r'^\s*tls_domain\s*=\s*"([^"\n]+)"', config, re.MULTILINE)
        tls_only = bool(re.search(r'^\s*tls\s*=\s*true', config, re.MULTILINE))
        if tls_only:
            if not match:
                raise RuntimeError('Telemt TLS domain is unavailable; retry when its API is ready')
            secret = 'ee' + secret + match.group(1).encode('utf-8').hex()
