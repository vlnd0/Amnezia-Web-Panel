...
from .serialization import serialized_manager
...
"""
    ...
    def _write_start_script(self, protocol_type, config_path=None):
        ...
        container_name = self._container_name(protocol_type)
        self.ssh.upload_file(self._with_port_redirects(self._render_start_script(protocol_type, config_path), protocol_type), "/tmp/_amnz_start.sh")
...
"""
    ...
    def _save_clients_table(self, protocol_type, clients_table):
        ...
        try:
            has_limits = any((c.get('userData') or {}).get('maxSpeed') for c in clients_table)
            _, _, limits_exist = self.ssh.run_sudo_command(
                f"docker exec -i {container_name} test -s {self._bwlimits_path()}"
            )
            if has_limits or limits_exist == 0:
...
"""
    ...
    def _get_reserved_ips(self, protocol_type):
        """Disabled clients retain their addresses until explicitly deleted."""
        result = set()
        for client in self._get_clients_table(protocol_type):
            ud = client.get('userData') or {}
            for value in (ud.get('clientIp'), ud.get('allowedIps')):
                ip = self._extract_ipv4(value or '')
                if ip:
                    result.add(ip)
        return result
    ...
    def _get_next_ip(self, protocol_type, extra_used=None):
        ...
        used = set()
        for ip in set(self._get_used_ips(protocol_type)) | self._get_reserved_ips(protocol_type):
            try:
...
"""
    ...
    def get_client_config(self, protocol_type, client_id, server_host, port):
        ...
        if not self._ip_in_subnet(client_ip, self._get_subnet(protocol_type)):
...
"""
    ...
    def toggle_client(self, protocol_type, client_id, enable):
        ...
        if enable:
            ...
            if client_ip and not self._ip_in_subnet(client_ip, self._get_subnet(protocol_type)):
                client_ip = None
            if client_ip:
...
"""
    ...
    def _build_run_cmd(self, protocol_type, published_ports, use_volume=True):
        container = self._container_name(protocol_type)
        ports = sorted({int(p) for p in published_ports})
        cmd = self._docker_run_cmd(container, container, ports[0], bool(self._get_subnet_ipv6_ip(protocol_type)))
        cmd = cmd.replace(f"-p {ports[0]}:{ports[0]}/udp", ' '.join(f"-p {p}:{p}/udp" for p in ports))
        if use_volume:
            cmd = cmd.replace('--name ', f'-v {self._volume_name(protocol_type)}:/opt/amnezia --name ')
        return cmd
    ...
    def reconfigure_ports(self, protocol_type, listen_port, published_ports):
        ...
        """
        if self._base_protocol(protocol_type) not in (self.AWG, self.AWG2, self.AWG_LEGACY):
    ...
    def _sync_config(self, protocol_type):
        ...
        if code != 0:
            raise RuntimeError(f"Failed to sync AWG peers: {err or out}")
        return dropped
...
"""
    ...
    def _with_port_redirects(self, script, protocol_type, listen_port=None, redirect_ports=None):
        listen_port = listen_port or self._current_listen_port(protocol_type)
        ports = self._current_published_ports(protocol_type) if redirect_ports is None else redirect_ports
        rules = '\n'.join(
            f"iptables -t nat -C PREROUTING -p udp --dport {int(p)} -j REDIRECT --to-ports {int(listen_port)} 2>/dev/null || "
            f"iptables -t nat -A PREROUTING -p udp --dport {int(p)} -j REDIRECT --to-ports {int(listen_port)}"
            for p in sorted(ports) if int(p) != int(listen_port))
        return script.replace('tail -f /dev/null', rules + '\ntail -f /dev/null') if rules else script

    def _build_start_script(self, protocol_type, network, listen_port=None, redirect_ports=None):
        # Upstream reads the subnet from Address at container start. Keep its
        # AWG3/userspace, IPv6 and exit-node handling when restoring port support.
        return self._with_port_redirects(self._render_start_script(protocol_type),
                                         protocol_type, listen_port, redirect_ports)

    def _ensure_subnet_nat(self, protocol_type, network):
        """Retain the fork's live /16 repair without restarting or replacing start.sh."""
        container = self._container_name(protocol_type)
        src = str(network)
        live = '; '.join(
        ...
        _, err, code = self.ssh.run_sudo_command(f"docker exec -i {container} sh -c '{live}'")
        if code != 0:
            raise RuntimeError(f"Cannot ensure AWG subnet routing: {err}")
        # Old fork scripts hardcode /16 or /24; upstream scripts read Address.
        # Add only the missing NAT rules, keeping all existing hooks/redirects.
        script, err, code = self.ssh.run_sudo_command(f"docker exec -i {container} cat /opt/amnezia/start.sh")
        if code != 0 or not script.strip():
            raise RuntimeError(f"Cannot read AWG start script: {err}")
        if src not in script and 'SUBNET=$(grep' not in script:
            if 'tail -f /dev/null' not in script:
                raise RuntimeError('Unrecognised AWG start script; refusing to replace it')
            script = script.replace('tail -f /dev/null', live + '\ntail -f /dev/null')
            self.ssh.upload_file(script, '/tmp/_amnz_start.sh')
            _, err, code = self.ssh.run_sudo_command(f"docker cp /tmp/_amnz_start.sh {container}:/opt/amnezia/start.sh")
            self.ssh.run_command('rm -f /tmp/_amnz_start.sh')
            if code != 0:
                raise RuntimeError(f"Cannot persist AWG subnet routing: {err}")
