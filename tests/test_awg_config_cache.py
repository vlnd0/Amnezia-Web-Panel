...
class FakeSSH:
    ...
    def run_sudo_command(self, command, timeout=60):
        ...
        probe = re.search(r"if test -f (\S+); then cat", command)
        if probe:
            path = probe.group(1)
            return (self.files[path], '', 0) if path in self.files else ('', '', 42)
...
class ServerConfigCacheTests(unittest.TestCase):
    def setUp(self):
        self.ssh = FakeSSH({CONFIG_PATH: SERVER_CONFIG, "/opt/amnezia/start.sh": "#!/bin/bash\n# 10.8.0.0/16\ntail -f /dev/null\n"})
    ...
    def test_next_ip_advances_after_peer_insert_on_same_manager(self):
        ...
        self.assertEqual(first, '10.8.0.1')
        ...
        self.assertEqual(self.manager._get_next_ip('awg'), '10.8.0.2')
