...
[ -n "$X_SUBNET" ] || X_SUBNET=10.8.1.1/16
...
if [ -f /opt/amnezia/awg/wg0.conf ]; then
  ...
  if ! ip link show "$IFACE" >/dev/null 2>&1; then
    ...
    if [ -n "$MOD_V" ] && [ -n "$TOOLS_V" ] && [ "$TOOLS_V" != "$MOD_V" ]; then
      ...
      if grep -q WG_FORCE_USERSPACE "$(command -v wg-quick)" 2>/dev/null; then
        echo "! retrying in userspace mode (amneziawg-go)"
        WG_FORCE_USERSPACE=1 wg-quick up /opt/amnezia/awg/wg0.conf
      fi
    fi
  fi
fi
